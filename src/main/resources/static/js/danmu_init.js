...
$("#img_botton").on("touchstart",function(){
    ...
    if(text !== ""){
        let newd={ "text":text , "color":"white" ,"size":"1","position":"0","time":time};
...
dataObj.then(value=>{
    ...
    for(let i = 0;i < bullets.length;i++){
        console.log(1);
        arr.push({ text: bullets[i].name + ": " + bullets[i].content, color: "#fef6e4", size: 1, position: 0, time: random + i * 100 });
    }
    // 添加初始弹幕
